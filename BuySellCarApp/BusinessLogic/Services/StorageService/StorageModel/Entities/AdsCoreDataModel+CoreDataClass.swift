...
public class AdsCoreDataModel: NSManagedObject {
    ...
    var adsImagesModel: [AdsImages] {
        let imagesSet = adsImages as? Set<AdsImages> ?? []
    ...
    convenience init(
        ...
        insertIntoManagedObjectContext context: NSManagedObjectContext) {
            ...
            let images: [AdsImages]? = adsDomainModel.adsImages?
                .map { .init(imageModel: $0, insertIntoManagedObjectContext: context) }
            
            self.adsImages = .init(array: images ?? [])
        }
}
